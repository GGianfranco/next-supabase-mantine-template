...
export function RegisterForm(props: PaperProps) {
  ...
  const handleSignInWithOtp = async (email: string) => {
    try {
      ...
      showNotificationInfo("Check your email for the register link!");
    } catch (error) {
      showNotificationError("Failed to send sign in link to email.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSignUpWithEmailAndPassword = async (
    ...
    try {
      ...
      showNotificationError("Failed to sign up with email and password");
    } finally {
      setLoading(false);
    }
  };
    ...
    <Paper radius="md" p="xl" withBorder {...props}>
      ...
      <form
        onSubmit={emailAndPasswordForm.onSubmit((values) =>
          handleSignUpWithEmailAndPassword(values.email, values.password)
        )}
      >
        <Stack>
          <TextInput
